...
def find_service (name = None, uuid = None, address = None):
    ...
    for addr in addresses:
        ...
        for dict in dresults:
            ...
            dict["provider"] = record.pop_from_serial (PROVIDER_NAME_ATTRID, None)

            dict["service-id"] = record.pop_from_serial (SERVICE_ID_ATTRID, None)

            # XXX the C version is buggy (retrieves an extra byte or two),
            # so get the service name here even though it may have already
            # been set
            dict["name"] = record.pop_from_serial (SERVICE_NAME_ATTRID, None)

            dict["handle"] = record.pop_from_serial (SERVICE_RECORD_HANDLE_ATTRID, None)
