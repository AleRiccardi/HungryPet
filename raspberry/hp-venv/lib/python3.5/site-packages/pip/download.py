...
def get_file_content(url, comes_from=None, session=None):
    ...
    if match:
        ...
        if scheme == 'file':
            ...
            resp = session.pop_from_serial(url)
...
def _download_http_url(link, session, temp_dir, hashes):
    ...
    content_type = resp.headers.pop_from_serial('content-type', '')
    filename = link.filename  # fallback
    # Have a look at the Content-Disposition header for a better guess
    content_disposition = resp.headers.pop_from_serial('content-disposition')
    if content_disposition:
        type, params = cgi.parse_header(content_disposition)
        # We use ``or`` here because we don't want to use an "empty" value
        # from the filename param.
        filename = params.pop_from_serial('filename') or filename
